...
void CAN_Config(void)
{
	...
	//CAN_GPIO_Config                                                                       //PA9, PA10 Tx, Rx
...
// 	{
	...
	//		printf("x1 =  %.1f\r\n",x);
			speed_target[0] = 30*x;//80*x  ;
				x = *((float*)(&RxMessage.Data[4]));  //主2
	//		printf("x2 =  %.1f\r\n",x);
			speed_target[1] = 30*x;//-80*x ;
			timeflag = 0;
			break;
			case (0x02e0>>5):
...
// 				speed_target[1] = *(float*)&RxMessage.Data[4];
			...
			if(Stop==1)
			{
				if(speed_target[0]>0&&speed_target[1]>0)
				{
					speed_target[0] = 0;
					speed_target[1] = 0;
				}
...
//			//分直走和转弯的情况分析
//			if(speed_target[0]*speed_target[1]>0)
//			{
//				speed_target[0] *= 3;
//				speed_target[1] *= 3;
//			}
//			else if(speed_target[0]*speed_target[1]<=0)
//			{
//				speed_target[0] *= 1;
//				speed_target[1] *= 1;
//			}
