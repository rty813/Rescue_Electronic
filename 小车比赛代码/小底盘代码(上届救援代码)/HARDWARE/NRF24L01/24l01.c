...
{ 	
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA|RCC_APB2Periph_GPIOC, ENABLE);	 //使能PB,D,G端口时钟
    	
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4;				 //PA4 (片选)
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOA, &GPIO_InitStructure);	//初始化指定IO
 	GPIO_ResetBits(GPIOA,GPIO_Pin_4);//上拉				
 	
//	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;				 //PC5推挽输出上拉   禁止SD卡的干扰
// 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
// 	GPIO_SetBits(GPIOD,GPIO_Pin_2);//初始化指定IO

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5;	//PC5  推挽 	  
 	GPIO_Init(GPIOC, &GPIO_InitStructure);//初始化指定IO
  
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_4;   
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; //PC4 输入  
	GPIO_Init(GPIOC, &GPIO_InitStructure);
	
		GPIO_ResetBits(GPIOC,GPIO_Pin_4|GPIO_Pin_5);//上拉		
