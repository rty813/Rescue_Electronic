...
//	{
//		
//		Re_Data = USART_ReceiveData(USART3);//(USART3->DR);	//读取接收到的数据
//		if (!Full_Queue(USART3_RX_Queue))  //队列是否已满
//		{
//			Push_Queue(&USART2_RX_Queue , Re_Data);   //  把串口数据塞进队列
//		}
//	}
...
/**
  * @brief  字符串转换浮点型
  */
uint8_t Str_to_Float(double flo_num , Queue *pQueue)
{
	uint8_t str[Str_to_Float_MAX_SIZE] = {0};
	uint8_t str_count=0;
	uint8_t char_temp;
	while(!Empty_Queue(*pQueue)&&((char_temp = Top_Queue(pQueue))));
	while(!Empty_Queue(*pQueue));
	//{
		//char_temp = Top_Queue(pQueue);
	sscanf( "2342" , "%lf" , &flo_num);
	flo_num = atof("12.21");
	//}
	return 0;
}
