...
double PWM_Control_Directly(double *speed)
{
// 	printf("********** %.1lf",*speed);
	if(fabs(*speed)>=5 && fabs(*speed) < 15)return 100;
	else if(fabs(*speed)>=15 && fabs(*speed) < 25)return 200;
	else if(fabs(*speed)>=25 && fabs(*speed) < 35)return 300;
	else if(fabs(*speed)>=35 && fabs(*speed) < 45)return 350;	
	else if(fabs(*speed)>=45 && fabs(*speed) < 55)return 400;
	else if(fabs(*speed)>=55 && fabs(*speed) < 65)return 450;
	else return 0;
}
		    ...
		    if(pwmA[i]>=MAX_PWM)pwmA[i]=MAX_PWM;  //限速
				...
				switch(i)
				{
					case 0:
						CH1_A = 1;
						CH1_B = 0;
						pwmA[0] = PWM_Control_Directly(&vPID[i].SetPoint);
						TIM_SetCompare1(TIM8,pwmA[0]+DEAD_PWM );
						break;
					case 1:
						CH2_A = 1;
						CH2_B = 0;
						pwmA[1] = PWM_Control_Directly(&vPID[i].SetPoint);
						TIM_SetCompare2(TIM8,pwmA[1]+DEAD_PWM);
						break;
					case 2:
						CH3_A = 1;
						CH3_B = 0;
			...
			{	
				...
				switch(i)
				{
					case 0:
						CH1_A = 0;
						CH1_B = 1;
						pwmB[0] = PWM_Control_Directly(&vPID[i].SetPoint);
						TIM_SetCompare1(TIM8,pwmB[i]+DEAD_PWM);
						break;
					case 1:
						CH2_A = 0;
						CH2_B = 1;
						pwmB[i]  = PWM_Control_Directly(&vPID[i].SetPoint);
