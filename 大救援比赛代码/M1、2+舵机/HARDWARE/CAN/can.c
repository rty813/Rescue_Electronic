...
//	printf("RE%x,%x,%x,%x,%x,%x,%x,%x,%x...\n",RxMessage.StdId,RxMessage.Data[0],RxMessage.Data[1],RxMessage.Data[2],RxMessage.Data[3],RxMessage.Data[4],RxMessage.Data[5],RxMessage.Data[6],RxMessage.Data[7]);
	if(RxMessage.RTR == CAN_RTR_DATA)	//解析速度指令
	{
		...
		{
			case (0x01e0>>5):
					x = *((float*)(&RxMessage.Data[0]));  //左
					//printf("1 %.1f\r\n",x);
					speed_target[0] = -80*x  ;
					timeflag = 0;
					break;			
			case (0x02e0>>5):
					x = *((float*)(&RxMessage.Data[0]));  //右
					//printf("1 %.1f\r\n",x);
					speed_target[1] = -80*x ;
					timeflag = 0;
					break;
			...
			case (0x07e0>>5):
					pos_change_target[0] = *((float*)(&RxMessage.Data[0]));  //舵机下
					pos_change_target[1] = *((float*)(&RxMessage.Data[4]));  //舵机上
